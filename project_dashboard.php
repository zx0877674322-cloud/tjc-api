...
$kpi_query = "SELECT 
    ...
    COALESCE(SUM(p.project_budget), 0) AS total_budget,
    
    -- (A) เซ็นสัญญา (ยังไม่เปิด PO)
    SUM(CASE WHEN p.status = 'เซ็นสัญญา' AND (SELECT COUNT(id) FROM purchase_orders WHERE project_id = p.id) = 0 THEN 1 ELSE 0 END) AS signed_projects,
    
    -- (B) กำลังดำเนินการจัดซื้อ (เซ็นสัญญา + มี PO แล้ว) [เพิ่มใหม่]
    SUM(CASE WHEN p.status = 'เซ็นสัญญา' AND (SELECT COUNT(id) FROM purchase_orders WHERE project_id = p.id) > 0 THEN 1 ELSE 0 END) AS purchasing_projects,
    
    -- (C) รอเซ็นสัญญา
    SUM(CASE WHEN p.status = 'รอเซ็นสัญญา' THEN 1 ELSE 0 END) AS waiting_projects,
    
    -- (D) แจ้งเตือนหมดสัญญา
    SUM(CASE 
        WHEN p.status = 'เซ็นสัญญา' AND p.end_date IS NOT NULL 
        THEN CASE 
            WHEN DATEDIFF(p.end_date, CURDATE()) <= COALESCE(NULLIF(p.alert_days_before_expire, 0), 30) THEN 1 
            ELSE 0 
        END 
        ELSE 0 
    END) AS expiring_projects,
    
    -- (E) แจ้งเตือนหมดประกัน
    SUM(CASE 
        WHEN p.status = 'เซ็นสัญญา' AND p.guarantee_end_date IS NOT NULL AND p.guarantee_end_date != '0000-00-00'
        THEN CASE 
            WHEN DATEDIFF(p.guarantee_end_date, CURDATE()) <= COALESCE(NULLIF(p.alert_warranty_days, 0), 30) AND DATEDIFF(p.guarantee_end_date, CURDATE()) >= 0 THEN 1 
...
// 1.2 ข้อมูล KPI แยกตามบริษัท (Logic เดียวกัน)
$kpi_company_query = "SELECT 
    ...
    SUM(CASE 
        ...
        THEN CASE 
            WHEN DATEDIFF(p.guarantee_end_date, CURDATE()) <= COALESCE(NULLIF(p.alert_warranty_days, 0), 30) AND DATEDIFF(p.guarantee_end_date, CURDATE()) >= 0 THEN 1 
            ELSE 0 END ELSE 0 END) AS expiring_warranty
FROM projects p GROUP BY p.company_id";
...
// 1.4 ดึงข้อมูลประเภทงานสำหรับทำ Dropdown ค้นหา
$jt_query = "SELECT type_name FROM project_job_types ORDER BY type_name ASC";
$jt_result = $conn->query($jt_query);
$job_types_filter = [];
if ($jt_result) {
    while ($r = $jt_result->fetch_assoc()) {
        $job_types_filter[] = $r['type_name'];
    }
}
...
$sql_list = "SELECT p.*, 
            ...
            -- 🟢 1. ดึงข้อมูลสถานะจากตาราง Master (Stages) โดยตรง
            COALESCE(s.stage_name, 'รอเซ็นสัญญา') as stage_name, 
            COALESCE(s.stage_color, 'badge-warning') as stage_color, 
            COALESCE(s.badge_icon, 'fa-clock') as badge_icon,
            ...
            cust.contact_phone as contact_phone,
        ...
        -- 🟢 2. JOIN ตาราง setup_project_stages ด้วย stage_id
        LEFT JOIN setup_project_stages s ON p.stage_id = s.id 
...
if (!$projects) {
    die("<div style='padding:20px; background:#fee2e2; color:#b91c1c; border-radius:8px; margin:20px;'>
            <b>🚨 พบข้อผิดพลาดใน SQL:</b><br> " . $conn->error . "
         </div>");
}
?>
...
<head>
    ...
    <style>
        body {
            ...
            /* ปรับพื้นหลังให้มีมิติ ไม่แบน และไม่ขาวจนเกินไป */
        ...
        .page-title {
            ...
            /* เพิ่มมิติให้ตัวอักษรนิดๆ */
        }

        .btn-create {
            /* ไล่สีให้ปุ่มดูมีมิติ และใส่เงาสะท้อนด้านบน */
            ...
            /* ตัดขอบปุ่ม */
            box-shadow: 0 4px 6px rgba(59, 130, 246, 0.4), inset 0 1px 0 rgba(255, 255, 255, 0.2);
        }

        .btn-create:hover {
            background: linear-gradient(135deg, #2563eb 0%, #1d4ed8 100%);
            transform: translateY(-2px);
            box-shadow: 0 6px 12px rgba(59, 130, 246, 0.5), inset 0 1px 0 rgba(255, 255, 255, 0.2);
        }
        ...
        .kpi-card {
            ...
            padding: 20px;
            border-radius: 16px;
            /* เพิ่มขอบชัดเจนและมิติเงาที่ลึกขึ้น */
            border: 1px solid #cbd5e1;
            box-shadow: 0 6px 12px rgba(0, 0, 0, 0.06), 0 2px 4px rgba(0, 0, 0, 0.04);
            display: flex;
            align-items: center;
            gap: 15px;
        ...
        .kpi-card.clickable:hover {
            ...
            box-shadow: 0 12px 20px rgba(0, 0, 0, 0.1), 0 4px 6px rgba(0, 0, 0, 0.05);
        }

        .kpi-card.active {
            box-shadow: 0 0 0 3px currentColor, 0 8px 15px rgba(0, 0, 0, 0.1);
            transform: translateY(-3px);
        }
        ...
        .kpi-icon {
            ...
            font-size: 1.5rem;
            flex-shrink: 0;
            /* มิติยุบลงไปนิดนึงให้ไอคอนเด่นขึ้น */
        ...
        .company-group {
            ...
            border-radius: 16px;
            padding: 24px;
            /* เพิ่มขอบและเงาให้ลอยขึ้นมาชัดเจน */
            border: 1px solid #cbd5e1;
        ...
        .company-group.active {
            border-color: #3b82f6;
            box-shadow: 0 10px 15px -3px rgba(59, 130, 246, 0.3), 0 0 0 1px #3b82f6;
        }
        ...
        .kpi-mini-card {
            /* ไล่สีให้ดูมีมิติ และตัดขอบให้ชัดเจนขึ้น */
        ...
        .table-card {
            ...
            border-radius: 16px;
            border: 1px solid #cbd5e1;
            /* ตัดขอบให้ตารางเด่นขึ้นจากพื้นหลัง */
            box-shadow: 0 8px 16px rgba(0, 0, 0, 0.06), 0 2px 6px rgba(0, 0, 0, 0.04);
            overflow: hidden;
        }
        ...
        td {
            ...
            /* ปรับเส้นตารางให้ชัดขึ้นกว่าเดิม */
        }
        ...
        .modal-content-box {
            ...
            border: 1px solid #cbd5e1;
            /* ตัดขอบ Modal */
            box-shadow: 0 25px 50px -12px rgba(0, 0, 0, 0.4), 0 0 0 1px rgba(255, 255, 255, 0.5) inset;
        ...
        /* ปรับ Grid ให้รองรับการ์ดเพิ่มขึ้น (จาก 5 เป็น 6 คอลัมน์) */
        .kpi-mini-grid-horizontal {
            grid-template-columns: repeat(6, 1fr);
            /* ปรับเป็น 6 */
        }
...
<body>
        ...
        <div class="all-company-dashboard-container">
            ...
            <div class="company-group active" id="compCard_all">
                ...
                <div class="kpi-mini-grid-horizontal">
                    ...
                    <div class="kpi-mini-card border-indigo text-indigo clickable mini-card-status"
                        data-status="กำลังดำเนินการจัดซื้อ"
                        onclick="filterByCompAndStat('all', 'กำลังดำเนินการจัดซื้อ')">
                        <div class="mini-title"><i class="fas fa-truck-loading"></i> กำลังจัดซื้อ/เปิด PO</div>
                        <div class="mini-value">
                            <?= number_format($kpi_all['purchasing_projects'] ?? 0) ?> <span
            ...
            <?php foreach ($companies as $comp):
                ...
                <div class="company-group" id="compCard_<?= $cid ?>">
                    ...
                    <div class="kpi-mini-grid">
                        ...
                        <div class="kpi-mini-card border-indigo text-indigo clickable mini-card-status hide-on-collapse"
                            data-status="กำลังดำเนินการจัดซื้อ"
                            onclick="filterByCompAndStat('<?= $cid ?>', 'กำลังดำเนินการจัดซื้อ')">
                            <div class="mini-title"><i class="fas fa-truck-loading"></i> กำลังจัดซื้อ/เปิด PO</div>
                            <div class="mini-value">
                                <?= number_format($cdata['purchasing_projects'] ?? 0) ?> <span
                                    style="font-size:0.9rem;color:#94a3b8;font-weight:400;">งาน</span>
                            </div>
                        </div>
        ...
        <div class="table-card">
            ...
            <div style="overflow-x: auto;">
                <table id="projectTable">
                    <thead>
                        <tr>
                            <th width="5%">หน้างาน</th>
                            <th width="5%">บริษัท</th>
                            <th width="8%" style="text-align: center;">สิ้นสุดสัญญา</th>
                            <th width="8%" style="text-align: center;">ครบกำหนดส่ง</th>
                            <th width="8%" style="text-align: center; color:#2563eb;">ระยะประกัน</th>
                            <th width="8%" style="text-align: center;">เริ่มค้ำฯ</th>
                            <th width="8%" style="text-align: center;">สิ้นสุดค้ำฯ</th>
                            <th width="15%">ชื่องบ/ชื่อโครงการ</th>
                            <th width="15%">หน่วยงาน</th>
                            <th width="10%">ประเภทงาน</th>
                            <th width="6%" style="text-align: center;">สถานะ</th>
                            <th width="4%" style="text-align: center;">จัดการ</th>
                        </tr>
                        <tr style="background: #f8fafc; border-bottom: 2px solid #cbd5e1;">
                            <th style="padding: 8px 10px;"><input type="text" id="colFilter0" onkeyup="filterTable()"
                                    class="form-control" placeholder="ค้นหา ID..."
                                    style="padding: 4px 8px; font-size: 0.8rem; border-radius: 4px;"></th>
                            <th></th>
                            <th></th>
                            <th></th>
                            <th></th>
                            <th></th>
                            <th></th>
                            <th style="padding: 8px 10px;"><input type="text" id="colFilter7" onkeyup="filterTable()"
                                    class="form-control" placeholder="ค้นหาโครงการ..."
                                    style="padding: 4px 8px; font-size: 0.8rem; border-radius: 4px;"></th>
                            <th style="padding: 8px 10px;"><input type="text" id="colFilter8" onkeyup="filterTable()"
                                    class="form-control" placeholder="ค้นหาหน่วยงาน..."
                                    style="padding: 4px 8px; font-size: 0.8rem; border-radius: 4px;"></th>
                            <th style="padding: 8px 10px;">
                                <select id="colFilter9" onchange="filterTable()" class="form-control"
                                    style="padding: 4px; font-size: 0.8rem; border-radius: 4px;">
                                    <option value="">-- ทั้งหมด --</option>
                                    <?php foreach ($job_types_filter as $jt): ?>
                    ...
                    <tbody>
                        <?php if ($projects && $projects->num_rows > 0): ?>
                            <?php if ($projects && $projects->num_rows > 0): ?>
                                <?php while ($row = $projects->fetch_assoc()):
                                    ...
                                    // 🟢 1. Logic สถานะ (แบบ Clean: ดึงจาก DB โดยตรง)
                                    $status_show = $row['stage_name'] ?? 'รอเซ็นสัญญา';
                                    $badge_class = $row['stage_color'] ?? 'badge-warning';
                                    $badge_icon = $row['badge_icon'] ?? 'fa-clock';

                                    // 2. จัดการข้อมูลสัญญา (ครบกำหนดส่ง)
                                    $is_signed = ($row['status'] == 'เซ็นสัญญา');
                                    $end_date_show = !empty($row['end_date']) ? date('d/m/Y', strtotime($row['end_date'])) : '-';

                                    $contract_countdown_html = "-";
                                    $is_row_expiring = false; // ตัวแปรเช็คสีแดง
                                    ...
                                    if (!empty($row['end_date']) && $row['end_date'] != '0000-00-00') {
                                        ...
                                        if ($today <= $expire_date) {
                                            ...
                                            if ($diff_c <= $alert_contract) {
                                                ...
                                                $contract_countdown_html = '<span class="badge expiring-badge" style="background:#fee2e2; color:#ef4444; border:1px solid #fca5a5;">' . $diff_c . ' วัน</span>';
                                            } else {
                                                $contract_countdown_html = '<span style="color:#10b981; font-weight:700;">' . $diff_c . ' วัน</span>';
                                            }
                                        } else {
                                            $is_row_expiring = true;
                                            $contract_countdown_html = '<span class="badge" style="background:#ef4444; color:#fff;">หมดสัญญา</span>';
                                        }
                                    }

                                    // 3. จัดการข้อมูลประกัน (ระยะประกัน)
                                    $warranty_countdown_html = "-";
                                    $is_warranty_warning = false; // ตัวแปรเช็คสีฟ้า
                                    ...
                                    if (!empty($row['guarantee_end_date']) && $row['guarantee_end_date'] != '0000-00-00') {
                                        ...
                                        if ($today <= $g_end_dt) {
                                            ...
                                            if ($diff_w <= $alert_warranty) {
                                                ...
                                                $warranty_countdown_html = '<span class="badge expiring-badge" style="background:#ffedd5; color:#ea580c; border:1px solid #fdba74;">' . $diff_w . ' วัน</span>';
                                            } else {
                                                $warranty_countdown_html = '<span style="color:#3b82f6; font-weight:700;">' . $diff_w . ' วัน</span>';
                                            }
                                        } else {
                                            $warranty_countdown_html = '<span class="badge" style="background:#cbd5e1; color:#fff;">หมดประกัน</span>';
                                        }
                                    }

                                    // จัดการข้อมูลวันที่เริ่ม-สิ้นสุดค้ำ
                                    ...
                                    // ✅✅✅ [จุดสำคัญ] Logic เลือกสีแถว (Priority: แดง > ฟ้า) ✅✅✅
                                    $row_class_name = '';
                                    if ($is_row_expiring) {
                                        $row_class_name = 'expiring-row'; // แดง (สำคัญสุด)
                                    } elseif ($is_warranty_warning) {
                                        $row_class_name = 'expiring-warranty-row'; // ฟ้า (รองลงมา)
                                    }
                                    ?>

                                    <tr class="<?= $row_class_name ?>" data-company-id="<?= htmlspecialchars($row['company_id']) ?>"
                        ...
                        /* เทคนิคตัดคำ 2 บรรทัดเหมือนชื่อลูกค้า ตารางจะได้ไม่พัง */
                        display: -webkit-box;
                        -webkit-line-clamp: 2; 
                        ...
                        white-space: normal;
                        line-height: 1.4;
                        min-width: 180px;" title="<?= htmlspecialchars($row['project_name']) ?>">
                                                <?= htmlspecialchars($row['project_name'] ?? '-') ?>
                                            </div>
                                        </td>

                                        <td>
                                            <div class="text-bold" style="color: #4f46e5; font-size:0.85rem; 
                                                   /* เทคนิคตัดคำ 2 บรรทัด */
                                                   display: -webkit-box;
                                                   -webkit-line-clamp: 2; /* โชว์สูงสุด 2 บรรทัด */
                                                   -webkit-box-orient: vertical;
                                                   overflow: hidden;
                                                   white-space: normal; /* อนุญาตให้ขึ้นบรรทัดใหม่ */
                                                   line-height: 1.4;    /* ระยะห่างบรรทัด */
                                                   min-width: 160px;" /* ขยายความกว้างขั้นต่ำนิดนึง */
                                        ...
                                        <td><span class="badge"
                                                style="background: #f1f5f9; color: #475569; border: 1px solid #cbd5e1; font-size: 0.75rem;"><?= htmlspecialchars($row['job_type_name']) ?></span>
                                        </td>

                                        <td class="status-col" style="text-align: center;">
                                            <span class="badge <?= $badge_class ?>" style="font-size: 0.75rem;">
                                                <i class="fas <?= $badge_icon ?>"></i> <?= htmlspecialchars($status_show) ?>
                                            </span>
                                        </td>
    ...
    <script>
        ...
        function applyFilters() {
            ...
            // ดึงค่าจากช่องค้นหารายคอลัมน์
            let valCol0 = document.getElementById("colFilter0") ? document.getElementById("colFilter0").value.toLowerCase() : "";
            let valCol7 = document.getElementById("colFilter7") ? document.getElementById("colFilter7").value.toLowerCase() : "";
            let valCol8 = document.getElementById("colFilter8") ? document.getElementById("colFilter8").value.toLowerCase() : "";
            let valCol9 = document.getElementById("colFilter9") ? document.getElementById("colFilter9").value.toLowerCase() : "";

            // เริ่มวนลูปแถวข้อมูล (ข้าม thead ซึ่งมี 2 แถว)
            for (let i = 2; i < tr.length; i++) {
                let row = tr[i];
                if (row.getElementsByTagName("td").length === 1) continue; // ข้ามแถว "ไม่พบข้อมูล"

                let cells = row.getElementsByTagName("td");

                // 0. ตรวจสอบ Company ID (จาก Data-Attribute)
                let matchCompany = (currentCompanyFilter === 'all') || (row.getAttribute('data-company-id') == currentCompanyFilter);

                // 1. ตรวจสอบเงื่อนไขสถานะ (KPI Card)
                let matchStatus = false;
                if (currentFilterStatus === 'all') {
                    matchStatus = true;
                } else if (currentFilterStatus === 'expiring') {
                    matchStatus = row.classList.contains('expiring-row');
                } else if (currentFilterStatus === 'expiring_warranty') {
                    matchStatus = row.classList.contains('expiring-warranty-row');
                } else {
                    let tdStatus = row.querySelector('.status-col');
                    if (tdStatus && tdStatus.textContent.trim() === currentFilterStatus) {
                        matchStatus = true;
                    }
                }

                // 2. ตรวจสอบเงื่อนไขช่วงวันที่ (Date Range)
                let matchDate = true;
                ...
                if (filterStart && (!rowStart || rowStart < filterStart)) matchDate = false;
                if (filterEnd && (!rowStart || rowStart > filterEnd)) matchDate = false;

                // -------------------------------------------------------------
                // 🟢 3. ตรวจสอบช่องค้นหาหลัก (Global Search)
                // ค้นหาจาก: หน้างาน (0), ชื่องบ/โครงการ (7), หน่วยงาน (8), ประเภทงาน (9)
                // -------------------------------------------------------------
                let matchSearch = false;
                if (searchText === "") {
                    matchSearch = true;
                } else {
                    let txt0 = cells[0] ? cells[0].textContent.toLowerCase() : "";
                    let txt7 = cells[7] ? cells[7].textContent.toLowerCase() : "";
                    let txt8 = cells[8] ? cells[8].textContent.toLowerCase() : "";
                    let txt9 = cells[9] ? cells[9].textContent.toLowerCase() : "";

                    if (txt0.includes(searchText) || txt7.includes(searchText) || txt8.includes(searchText) || txt9.includes(searchText)) {
                        matchSearch = true;
                    }
                }

                // -------------------------------------------------------------
                // 🟢 4. ตรวจสอบช่องค้นหารายคอลัมน์ (Column Filters)
                // -------------------------------------------------------------
                let matchCol0 = true, matchCol7 = true, matchCol8 = true, matchCol9 = true;

                if (valCol0 && cells[0] && !cells[0].textContent.toLowerCase().includes(valCol0)) matchCol0 = false;
                if (valCol7 && cells[7] && !cells[7].textContent.toLowerCase().includes(valCol7)) matchCol7 = false;
                if (valCol8 && cells[8] && !cells[8].textContent.toLowerCase().includes(valCol8)) matchCol8 = false;
                if (valCol9 && cells[9] && !cells[9].textContent.toLowerCase().includes(valCol9)) matchCol9 = false;

                // -------------------------------------------------------------
                // แสดงแถวก็ต่อเมื่อตรงกับ "ทุกเงื่อนไข"
                // -------------------------------------------------------------
                if (matchCompany && matchStatus && matchDate && matchSearch && matchCol0 && matchCol7 && matchCol8 && matchCol9) {
                    row.style.display = "";
                } else {
