...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($r_date) || empty($r_note)) {
        echo json_encode(['status' => 'error', 'message' => 'กรุณากรอกรายละเอียดการจัดส่ง']);
        exit;
    }

    $stmt = $conn->prepare("INSERT INTO delivery_daily_reports (report_date, report_time, reporter_id, note) VALUES (?, ?, ?, ?)");
...
<head>
    ...
    <style>
        ...
        .main-content {
            margin-left: 250px; /* ปรับกลับเป็น 250px เพื่อให้พอดีกับ Sidebar ปกติ */
            padding: 25px;
        ...
        .modern-input:focus { border-color: var(--primary); outline: none; box-shadow: 0 0 0 4px rgba(245, 158, 11, 0.1); }
        .modern-input[readonly] { background-color: #f1f5f9; color: #64748b; cursor: not-allowed; }

        .btn-submit {
            width: 100%; padding: 14px; background: var(--primary); color: #fff;
            border: none; border-radius: 10px; font-size: 1rem; font-weight: 600;
            cursor: pointer; transition: 0.2s; box-shadow: 0 4px 6px rgba(245, 158, 11, 0.2);
...
<div class="main-content">
    ...
    <div class="page-header">
        <h2><i class="fas fa-truck-fast" style="color:var(--primary);"></i> บันทึกงานจัดส่ง</h2>
        <p>Delivery Daily Report Log</p>
    </div>
    ...
    <div class="report-card">
        <form id="deliveryForm" onsubmit="return false;">
            ...
            <div class="form-group">
                <label class="form-label">ผู้ขับรถ / ผู้ส่งงาน</label>
            ...
            <div class="form-group">
                <label class="form-label">รายละเอียดเส้นทาง / งานที่ส่ง / ปัญหาที่พบ</label>
                <textarea name="note" class="modern-input" rows="5" placeholder="เช่น ส่งสินค้าบ.ไทยรุ่งเรือง เสร็จสิ้น, รถติดมากเส้นพระราม 2..."></textarea>
            </div>

            <button class="btn-submit" onclick="saveReport()">
                <i class="fas fa-save"></i> บันทึกข้อมูลจัดส่ง
            </button>
        </form>
    </div>
</div>
...
<script>
    $(document).ready(function() {
        ...
        flatpickr(".date-picker", {
            altInput: true,
            altFormat: "d/m/Y",  // [แก้ไขเรียบร้อย]
            dateFormat: "Y-m-d", // ส่งค่าเข้า DB เป็น 2026-02-09
            locale: "th",
            defaultDate: "today"
        });
    ...
    function saveReport() {
        let formData = new FormData(document.getElementById('deliveryForm'));
        ...
        $.ajax({
            url: 'delivery_create_report.php',
            type: 'POST',
            data: formData,
