...
// [แก้ไข] เริ่มต้นด้วย WHERE 1=1 (เพื่อให้เติมเงื่อนไขอื่นต่อท้ายได้ง่าย)
$where_sql = " WHERE 1=1 ";

// ถ้ามีการเลือกวันที่ (ไม่เป็นค่าว่าง) ให้เพิ่มเงื่อนไขกรองวันที่
...
// 2.1 ดึงรายชื่อพนักงาน (เฉพาะที่มีข้อมูล)
$users_opt = [];
$sql_emp = "SELECT DISTINCT u.id, u.fullname 
            FROM users u 
            INNER JOIN hr_daily_reports h ON u.id = h.reporter_id 
            ORDER BY u.fullname ASC";
$res_u = $conn->query($sql_emp);
while ($u = $res_u->fetch_assoc())
    $users_opt[] = $u;

// 2.2 ดึงรายการรายงาน HR (ใช้ตัวแปร $res_list ให้ตรงกับ HTML ด้านล่าง)
...
<body>
    ...
    <div class="main-content">
        ...
        <div style="margin-bottom: 25px;">
            <h2 style="margin:0; font-weight:700; color:var(--text-main);">รายงานประจำวัน</h2>
            <p style="margin:5px 0 0; color:var(--text-muted);">ระบบค้นหาและตรวจสอบรายงานประจำวัน</p>
        </div>
        ...
        <div class="filter-section">
            <form class="filter-form" method="GET">
                ...
                <a href="hr_dashboad.php" class="btn-reset"><i class="fas fa-redo"></i></a>
            </form>
        </div>
                ...
                <table>
                    <thead>
                        <tr>
                            ...
                            <th width="20%">ผู้รายงาน</th>
                            <th width="55%">รายละเอียด (Note)</th>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="status-badge" style="background:#e0e7ff; color:#4338ca;">
                                            <i class="fas fa-user-circle"></i> <?= htmlspecialchars($row['fullname']) ?>
                                        </div>
                                    </td>
                                    <td>
                                        <div
                                            style="display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; font-size:0.9rem; color:var(--text-muted); line-height:1.5;">
    ...
    <div id="detailModal" class="modal">
        <div class="modal-content">
            <div class="modal-header">
                <h3><i class="fas fa-file-alt" style="color:var(--primary); margin-right:8px;"></i> รายละเอียดรายงาน
                </h3>
    ...
    <script>
        ...
        function showDetail(data) {
            ...
            <div style="text-align:right; margin-top:15px; font-size:0.8rem; color:#94a3b8;">
                Record ID: #${data.id} | Timestamp: ${data.created_at}
            </div>
        `;
