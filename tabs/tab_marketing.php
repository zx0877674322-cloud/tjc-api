...
// [Delete Logic] สำหรับ Tab นี้โดยเฉพาะ
if (isset($_GET['delete_id']) && isset($_GET['tab']) && $_GET['tab'] == 'marketing') {
    $del_id = intval($_GET['delete_id']);
    $conn->query("DELETE FROM $table_name WHERE id = $del_id");
    echo "<script>window.location.href='StaffHistory.php?tab=marketing';</script>";
    exit();
}

// =========================================================
// 2. FILTER LOGIC (แก้ไขสมบูรณ์: ใช้ SESSION['fullname'])
// =========================================================

// 1. ดึงชื่อผู้ใช้ (แก้ไขจุดสำคัญตรงนี้!)
if (isset($staff_name) && !empty($staff_name)) {
    // กรณีดูในฐานะ Admin หรือส่งค่ามาจากหน้าอื่น
    $target_user = $staff_name;
} elseif (isset($_SESSION['fullname']) && !empty($_SESSION['fullname'])) {
    // ✅ กรณี Login ปกติ ใช้ 'fullname' ตามที่คุณแจ้งมา
    $target_user = $_SESSION['fullname'];
} elseif (isset($_SESSION['name']) && !empty($_SESSION['name'])) {
    // เผื่อไว้ (Fallback) กรณีระบบเก็บใน 'name'
    $target_user = $_SESSION['name'];
} else {
    $target_user = '';
}

$target_user = trim($target_user); // ตัดช่องว่างหน้าหลังออก

// รับค่าตัวแปรวันที่และการค้นหา
...
// ✅ 2. กรองชื่อ (สำคัญที่สุด)
if (!empty($target_user)) {
    $safe_name = $conn->real_escape_string($target_user);
    $base_where .= " AND reporter_name = '$safe_name'";
} else {
    // ถ้าหาชื่อไม่เจอ (Session หลุด) ให้ปิดการแสดงผลเพื่อความปลอดภัย
    $base_where .= " AND 1=0 "; 
}

// ✅ 3. กรองวันที่ (เขียนแบบยืดหยุ่น)
// ถ้ามีทั้งวันที่เริ่ม และ วันที่สิ้นสุด ให้ใช้ BETWEEN ตามที่คุณต้องการ
if (!empty($start_date) && !empty($end_date)) {
    $base_where .= " AND report_date BETWEEN '$start_date' AND '$end_date'";
} 
// เผื่อกรณีเลือกแค่วันใดวันหนึ่ง
elseif (!empty($start_date)) {
    $base_where .= " AND report_date >= '$start_date'";
}
elseif (!empty($end_date)) {
    $base_where .= " AND report_date <= '$end_date'";
}

// 4. กรองคำค้นหา
if ($keyword) {
    $base_where .= " AND (platform_name LIKE '%$keyword%' OR order_number LIKE '%$keyword%' OR item_details LIKE '%$keyword%')";
}

// =========================================================
// 3. KPI & PLATFORM CALCULATION
// =========================================================
$status_counts = []; 
$platform_stats = []; 
$platform_info = [];
...
// Helper Function
if (!function_exists('getMktTabStatusConfig')) {
    function getMktTabStatusConfig($status) {
        if (strpos($status, 'ดำเนินการ')!==false || strpos($status, 'รอ')!==false || strpos($status, 'Pending')!==false) return ['bg'=>'var(--bg-warning-soft)', 'text'=>'var(--text-warning)', 'border'=>'var(--warning)', 'icon'=>'fa-clock'];
        if (strpos($status, 'ส่งแล้ว')!==false || strpos($status, 'สำเร็จ')!==false || strpos($status, 'Complete') !== false) return ['bg'=>'var(--bg-success-soft)', 'text'=>'var(--text-success)', 'border'=>'var(--success)', 'icon'=>'fa-check-circle'];
        if (strpos($status, 'ตีกลับ')!==false || strpos($status, 'ยกเลิก')!==false || strpos($status, 'Fail') !== false) return ['bg'=>'var(--bg-danger-soft)', 'text'=>'var(--text-danger)', 'border'=>'var(--danger)', 'icon'=>'fa-times-circle'];
        if (strpos($status, 'ขนส่ง')!==false || strpos($status, 'Tracking')!==false) return ['bg'=>'var(--bg-info-soft)', 'text'=>'var(--text-info)', 'border'=>'var(--info)', 'icon'=>'fa-truck'];
        $hash = crc32($status); 
        $palettes = [['name'=>'purple','icon'=>'fa-cube'],['name'=>'pink','icon'=>'fa-gift'],['name'=>'teal','icon'=>'fa-clipboard-check'],['name'=>'cyan','icon'=>'fa-tasks'],['name'=>'indigo','icon'=>'fa-layer-group'],['name'=>'rose','icon'=>'fa-box-open']];
        $p = $palettes[abs($hash)%count($palettes)];
        return ['bg'=>"var(--bg-{$p['name']}-soft)", 'text'=>"var(--text-{$p['name']})", 'border'=>"var(--{$p['name']})", 'icon'=>$p['icon']];
    }
}
?>
...
<style>
    ...
    /* Financial Cards */
    .financial-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 20px; margin-bottom: 30px; }
    .fin-card { background: var(--bg-card); padding: 25px; border-radius: var(--radius-xl); border: 1px solid var(--border-color); box-shadow: var(--shadow-sm); display: flex; justify-content: space-between; align-items: center; cursor: pointer; height: 120px; transition: transform 0.2s; }
    ...
    .fin-value { font-size: 32px; font-weight: 800; line-height: 1; }
    .fin-icon-bg { width: 56px; height: 56px; border-radius: 12px; display: flex; align-items: center; justify-content: center; font-size: 26px; }
    /* Status Mini Grid */
    ...
    .mini-card { background: var(--bg-card); padding: 15px 20px; border-radius: 14px; border: 1px solid var(--border-color); box-shadow: var(--shadow-sm); cursor: pointer; display: flex; flex-direction: column; justify-content: center; height: 100px; transition: transform 0.2s; }
    ...
    /* Filters */
    .filter-wrapper { background: var(--bg-card); padding: 25px; border-radius: var(--radius-xl); border: 1px solid var(--border-color); margin-bottom: 30px; }
    .filter-form { display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 15px; align-items: end; }
    .form-control { width: 100%; padding: 10px 14px; border-radius: 8px; border: 1px solid var(--border-color); background: var(--bg-input); color: var(--text-main); font-family: 'Prompt'; }
    .btn-action { display: inline-flex; align-items: center; justify-content: center; height: 42px; padding: 0 20px; border-radius: 8px; font-weight: 600; border: none; cursor: pointer; width: 100%; gap: 8px; }
    .btn-primary { background: var(--primary); color: #fff; }
    .btn-outline { background: transparent; border: 1px solid var(--border-color); color: var(--text-muted); }
    /* Table */
    .table-container { background: var(--bg-card); border-radius: var(--radius-xl); border: 1px solid var(--border-color); overflow: hidden; }
    ...
    td { padding: 16px 20px; border-bottom: 1px solid var(--border-color); vertical-align: middle; color: var(--text-main); font-size: 14px; }
    ...
    .btn-icon { width: 34px; height: 34px; border-radius: 8px; display: inline-flex; align-items: center; justify-content: center; border: 1px solid var(--border-color); color: var(--text-muted); cursor: pointer; text-decoration: none; }
    ...
    .modal-content { background: var(--bg-card); margin: 3vh auto; border-radius: 20px; width: 95%; max-width: 850px; max-height: 94vh; overflow-y: auto; box-shadow: 0 10px 15px -3px rgba(0,0,0,0.2); border: 1px solid var(--border-color); color: var(--text-main); }
    ...
    .receipt-table, .detail-table { width: 100% !important; min-width: 400px; border-collapse: collapse; font-size: 13px; }
    .receipt-table th, .detail-table th, .receipt-table td, .detail-table td { padding: 8px 12px !important; white-space: nowrap; vertical-align: middle; }
    .table-fit-wrapper { overflow-x: auto; background: var(--bg-card); padding: 0; display: block; }
...
<div style="margin-bottom: 20px;">
    <h3 style="margin:0; font-weight:700; color:var(--text-main);">ประวัติการบันทึก: <span style="color:var(--primary);"><?php echo htmlspecialchars($target_user); ?></span></h3>
</div>
...
<div style="margin-bottom: 30px; background: var(--bg-card); border-radius: var(--radius-xl); border: 1px solid var(--border-color); overflow: hidden; box-shadow: var(--shadow-sm);">
    <div onclick="document.getElementById('platformSection').style.display = document.getElementById('platformSection').style.display === 'none' ? 'block' : 'none'" style="padding: 15px 25px; background: var(--bg-subtle); cursor: pointer; display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid var(--border-color);">
        <div style="display:flex; align-items:center; gap:12px;">
            ...
            <h4 style="margin:0; font-weight: 700; font-size:16px; color: var(--text-main);">ยอดขายแยกตามร้านค้า (ของคุณ)</h4>
...
<div class="financial-grid">
    ...
    <div class="fin-card" onclick="filterByStatus('')">
        ...
        <div class="fin-icon-bg" style="background: var(--bg-danger-soft); color: var(--danger);">
            <i class="fas fa-wallet"></i>
        </div>
    </div>
</div>

<div class="section-divider"><i class="fas fa-tasks"></i> สถานะออเดอร์</div>

<div class="status-grid">
    <?php foreach($status_counts as $st_name => $count): 
        $c = getMktTabStatusConfig($st_name); 
        $isActive = ($filter_status == $st_name) ? 'active-filter' : '';
        $cardStyle = "background-color: {$c['bg']}; color: {$c['text']}; border-color: transparent;";
        if($isActive) { $cardStyle = "background-color: var(--bg-card); color: {$c['text']}; border-color: {$c['border']}; box-shadow: 0 0 0 1px {$c['border']};"; }
    ?>
    <div class="mini-card <?php echo $isActive; ?>" onclick="filterByStatus('<?php echo $st_name; ?>')" style="<?php echo $cardStyle; ?>">
        <div class="mini-top">
            <span class="mini-label"><?php echo htmlspecialchars($st_name); ?></span>
            <i class="fas <?php echo $c['icon']; ?>" style="opacity:0.8;"></i>
        </div>
        <div class="mini-value"><?php echo number_format($count); ?></div>
    </div>
    <?php endforeach; ?>
...
<div class="filter-wrapper">
    <form class="filter-form" method="GET" action="" id="filterForm">
        ...
        <div><label class="form-label">ค้นหา</label><input type="text" name="keyword" value="<?php echo $keyword; ?>" class="form-control" placeholder="Platform, Order No..."></div>
        <div><label class="form-label">วันที่เริ่ม</label><input type="date" name="start_date" value="<?php echo $start_date; ?>" class="form-control"></div>
        <div><label class="form-label">ถึงวันที่</label><input type="date" name="end_date" value="<?php echo $end_date; ?>" class="form-control"></div>
        
        <div style="display:flex; gap:10px;">
            <button type="submit" class="btn-action btn-primary"><i class="fas fa-search"></i> ค้นหา</button>
            <a href="?tab=marketing" class="btn-action btn-outline" style="width:auto;"><i class="fas fa-undo"></i></a>
        </div>
    </form>
</div>
        ...
        <table>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><?php echo $row['order_number'] ? str_replace(',', '<br>', $row['order_number']) : '-'; ?></td>
                    <td><?php echo $row['doc_references'] ? str_replace(',', '<br>', $row['doc_references']) : '-'; ?></td>
                    ...
                    <td style="text-align:center;">
                        <button onclick='showMktDetail(<?php echo htmlspecialchars(json_encode($row, JSON_HEX_APOS | JSON_HEX_QUOT | JSON_INVALID_UTF8_IGNORE), ENT_QUOTES, 'UTF-8'); ?>)' class="btn-icon" title="ดูรายละเอียด"><i class="fas fa-eye"></i></button>
                        <a href="Online_Marketing_Report.php?id=<?php echo $row['id']; ?>" class="btn-icon"><i class="fas fa-edit"></i></a>
                        <a href="?tab=marketing&delete_id=<?php echo $row['id']; ?>" onclick="return confirm('ยืนยันลบ?')" class="btn-icon" style="color:var(--danger); border-color:var(--border-danger-soft);"><i class="fas fa-trash-alt"></i></a>
                    </td>
                </tr>
                <?php endwhile; else: echo "<tr><td colspan='8' style='text-align:center; padding:50px; color:var(--text-muted);'>ไม่พบข้อมูลประวัติของคุณ</td></tr>"; endif; ?>
            </tbody>
        </table>
    </div>
</div>
...
<div id="detailModal" class="modal" onclick="if(event.target==this)closeModal()">
    <div class="modal-content">
        <div class="modal-header">
            ...
            <button onclick="closeModal()" style="width: 32px; height: 32px; border-radius: 50%; border: none; background: var(--bg-subtle); color: var(--text-muted); cursor: pointer;"><i class="fas fa-times"></i></button>
        </div>
...
<div id="imageViewer" class="image-viewer" onclick="closeImageViewer()">
    <span class="image-viewer-close" style="position:absolute; top:20px; right:30px; color:white; font-size:40px; cursor:pointer;">&times;</span>
    <img id="viewerImage" src="">
    <div id="viewerCaption" style="margin-top:15px; color:white; font-size:16px;"></div>
</div>
            ...
            shops.forEach((shopTxt, idx) => {
                ...
                html += `<div class="receipt-card">
                            <div class="receipt-header">
                                <div style="font-weight:700;"><i class="fas fa-store" style="color:var(--primary);"></i> ${displayName} <span style="background:rgba(0,0,0,0.05); padding:2px 8px; border-radius:4px; font-size:12px; color:var(--text-muted); border:1px solid var(--border-color);">#${orderNo}</span></div>
                                <span style="font-size:12px; padding:4px 12px; border-radius:20px; font-weight:700; background:var(--bg-subtle); color:var(--text-muted);">${st}</span>
                            </div>
