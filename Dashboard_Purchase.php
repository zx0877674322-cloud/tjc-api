...
while ($row = $res_raw->fetch_assoc()) {
    ...
    foreach ($raws as $r) {
        ...
        if ($clean)
            $status_counts[$clean] = ($status_counts[$clean] ?? 0) + 1;
    }
}
ksort($status_counts);
...
<head>
    ...
    <style>
        ...
        * {
        ...
        .shop-head {
            background: var(--bg-shop-head);
            padding: 12px 20px;
            font-weight: 700;
            color: var(--text-shop-head);
            font-size: 14px;
            border-bottom: 1px solid var(--border-color);
            display: flex;
            align-items: center;
        ...
        .note-box {
            background: var(--bg-card);
            /* ปรับ Padding: บนล่าง 10px, ซ้ายขวา 15px (ลดจากเดิม 15px) */
            padding: 10px 15px; 
            border-radius: 12px;
            border: 1px solid var(--border-color);
            margin-top: 15px;
            ...
            /* ❌ ลบ 2 บรรทัดนี้ออกจากตรงนี้ครับ */
            /* white-space: pre-wrap; */
            /* word-break: break-word; */
        }

        /* ✅ เพิ่มอันนี้เข้าไปใหม่ครับ จะเป็นการสั่งเฉพาะตัวข้อความเนื้อหา */
        .note-box div:last-child {
            white-space: pre-wrap;   /* ให้ขึ้นบรรทัดใหม่ตามที่พิมพ์ */
            word-break: break-word;  /* ตัดคำยาวๆ ไม่ให้ทะลุ */
            margin-top: 5px;         /* เว้นระยะห่างจากหัวข้อ 'หมายเหตุ' นิดหน่อย */
        }
    ...
    <div class="main-container">
        ...
        <div class="kpi-grid">
            <div class="kpi-card">
                ...
                <div class="kpi-value" style="color:var(--primary-color);"><?php echo number_format($total_reports); ?>
                </div>
            ...
            <?php foreach ($status_counts as $st => $cnt):
                ...
                <div class="kpi-card">
                    ...
                    <div class="kpi-value" style="color:<?php echo $conf['text']; ?>;"><?php echo number_format($cnt); ?>
                    </div>
        ...
        <form class="filter-wrapper" method="GET">
            <div class="filter-header"><i class="fas fa-list-ul" style="color:var(--primary-color);"></i>
                รายการจัดซื้อของฉัน</div>
            <div class="filter-form">
                <input type="text" name="filter_name" value="<?php echo $filter_name; ?>" class="form-input"
                    placeholder="ค้นหา: พนักงาน..." style="min-width: 150px;">

                <select name="filter_status" class="form-input" style="min-width: 150px;">
                    <option value="">-- สถานะบิลทั้งหมด --</option>
                    <?php foreach ($status_keys as $s) {
                        echo "<option value='$s' " . ($filter_status == $s ? 'selected' : '') . ">$s</option>";
                    } ?>
                </select>
                                ...
                                <tr>
                                    <td>
                                        <div style="font-weight:700; color:var(--text-main);">
                                            <?php echo date('d/m/Y', strtotime($row['report_date'])); ?></div>
                                        <div style="font-size:12px; color:var(--text-muted); margin-top:2px;">
                                            <?php echo date('H:i', strtotime($row['created_at'])); ?> น.</div>
                                    </td>
                                    <td>
                                        <div style="display:flex; align-items:center; gap:8px; white-space:nowrap;">
                                            <span
                                                style="font-weight:600; color:var(--text-main);"><?php echo $row['reporter_name']; ?></span>
                                        </div>
                                    </td>
                                    <td><span
                                            style="color:var(--primary-color); font-weight:700;"><?php echo $row['supplier_name']; ?></span>
                                    </td>
                                    <td><?php echo $row['project_name']; ?></td>
                                    <td style="text-align:center;"><span
                                            style="background:var(--bg-hover); color:var(--text-muted); padding:3px 8px; border-radius:6px; font-weight:600; font-size:12px; border:1px solid var(--border-color);"><?php echo $row['item_count']; ?></span>
                                    </td>
                                    <td>
                                    ...
                                    <td style="font-weight:700; color:#ef4444; font-size:15px; text-align:right;">
                                        <?php echo number_format($row['total_expense']); ?></td>
                                    ...
                                    <td style="text-align:center;"><button
                                            onclick='showPurchaseDetail(<?php echo json_encode($row); ?>)' class="btn-view"><i
                                                class="fas fa-eye"></i></button></td>
                                </tr>
                            <?php endwhile; else:
                            echo "<tr><td colspan='9' style='text-align:center; padding:40px; color:var(--text-muted);'>ไม่พบข้อมูล</td></tr>"; endif; ?>
                    </tbody>
                </table>
            </div>
        </div>
    ...
    <div id="purchaseModal" class="modal">
        <div class="modal-content">
            <div class="modal-header">
                <div class="modal-title"><i class="fas fa-shopping-cart" style="color:var(--primary-color);"></i>
                    รายละเอียดการเบิกจ่าย</div>
